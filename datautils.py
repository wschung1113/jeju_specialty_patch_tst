...
# add your custom dataset here for assertion
DSETS = ['ettm1', 'ettm2', 'etth1', 'etth2', 'electricity',
         'traffic', 'illness', 'weather', 'exchange', 'jeju_specialty', 
        ]
...
def get_dls(params):
    ...
    # must add this elif clause for custom dataset
    elif params.dset == 'jeju_specialty':
        root_path = './dataset/'
        size = [params.context_points, 0, params.target_points]
        dls = DataLoaders(
                datasetCls=Dataset_Jeju,
                dataset_kwargs={
                'root_path': root_path,
                ...
                'size': size,
                'use_time_features': params.use_time_features,
                # custom dataset의  time column명을 지정해줘야함
