...
class PatchTST(nn.Module):
    # n_vars is prediction horizon length? ==> had it other way around. target_dim seems to be the prediction horizon length and n_vars the number of independent time series channels
    """
    Output dimension: 
         [bs x target_dim x n_vars] for prediction
...
class PatchTSTEncoder(nn.Module):
    def __init__(self, c_in, num_patch, patch_len, 
        ...
        # 쪼개진 univariate time series patch들을 embedding 하는 레이어 생성
        # shared_embedding은 patch 들을 공유 linear layer로 embedding 할지 각 patch마다 embedding linear layer를 둘지 여부
