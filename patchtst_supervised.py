...
parser.add_argument('--c_in', type=int, default=1, help='number of timeseries input; 1 for univariate')
...
# training
parser.add_argument('--weight_path', type=str, default='', help='path to saved weight')
...
def pred_func():
    # get dataloader
    dls = get_dls(args)
    model = get_model(args.c_in, args)
    # print(model)
    #model = torch.load(weight_path)
    # get callbacks
...
if __name__ == '__main__':
    if args.is_train == 1:   # training mode
        suggested_lr = find_lr()
        print('suggested lr:', suggested_lr)
        train_func(suggested_lr)
    elif args.is_train == 0:   # testing mode
        out = test_func()
        print('score:', out[2])
        print('shape:', out[0].shape)
    else: # args.is_train == 2; prediction mode
        out = pred_func()
        print(out)
        print(out.shape)
        # print('shape:', out[0].shape)
